...
enum TransactionCommand(val commandStartText: String, val description: String):
  case Income      extends TransactionCommand("/доход ", "добавить сумму к бюджету")
  case Expenditure extends TransactionCommand("/расход ", "удалить сумму из бюджета")
...
object CommandParser:
  def toAction(text: String): Action =
    parse(text) match 
      ...
      case TransactionCommand.Expenditure => Action.fromTransactionCommand(text)
